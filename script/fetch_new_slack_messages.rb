...
begin
  ...
  connection.logger.info "Checking searchd status."
  ...
  if r.read == "The Sphinx daemon searchd is not currently running.\n"
    # start searchd
    connection.logger.info "searchd is not running. Starting searchd."
    ...
    if status
      connection.logger.info "searchd started."
    ...
    connection.logger.info "searchd is running."
  end
  ...
  Rails.logger.error("Caught exception in fetch_new_slack_messages.rb; exiting")
